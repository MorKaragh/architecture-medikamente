...
flowchart LR
  subgraph prep [Подготовка]
    A[Базовая инфраструктура и ИБ]
    B[Миграция исторических данных]
    C[Интеграции в режиме shadow]
  end
  subgraph waves [Волны parallel run]
    W1[Запись и расписание]
    W2[EMR и вложения]
    W3[Оплаты и ККМ]
    W4[Склад и ТМЦ]
    W5[Аналитика read-only]
  end
  subgraph close [Закрытие]
    Z[Отключение дублирующих Excel-потоков]
    R[Архив AS-IS и пост-аудит]
  end
  prep --> W1 --> W2 --> W3 --> W4 --> W5 --> Z --> R
```
