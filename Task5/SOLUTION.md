...
**AS-IS.** Данные размазаны по журналам Excel, сканам на общем диску, дублирующим таблицам у касс; 1С «Бухгалтерия» и «Торговля и склад» в файловом режиме; ККМ связаны с 1С; нет единого аудита и формализованных API на границах доменов.
...
| **Branch by abstraction** | За фасадом постепенно меняют реализацию (legacy → новый сервис) | Хорошо для API и кода; снижает связность | Для Excel/сканов как основного носителя требует предварительно вынести данные в управляемые хранилища и контракты; один фасад не закрывает обучение 200+ ролей в организации |
...
```mermaid
flowchart TD
  A[Подготовка: инфраструктура, ИБ, мониторинг] --> B[Миграция исторических данных и сверка]
  B --> C[Интеграции в shadow-режиме]
  C --> D[T1: Параллельный запуск записи и расписания]
  D --> E{Go/No-Go по T1}
  E -->|Go| F[T2: Параллельный запуск EMR и вложений]
  E -->|No-Go| R1[Откат T1 и корректирующие действия]
  R1 --> D
  F --> G{Go/No-Go по T2}
  G -->|Go| H[T3: Параллельный запуск оплат и ККМ]
  G -->|No-Go| R2[Откат T2 и корректирующие действия]
  R2 --> F
  H --> I{Go/No-Go по T3}
  I -->|Go| J[T4: Параллельный запуск склада и ТМЦ]
  I -->|No-Go| R3[Откат T3 и корректирующие действия]
  R3 --> H
  J --> K{Go/No-Go по T4}
  K -->|Go| L[T5: Нагрузочные и пользовательские тесты]
  K -->|No-Go| R4[Откат T4 и корректирующие действия]
  R4 --> J
  L --> M{Критерии стабильности выполнены}
  M -->|Да| N[Отключение Excel-дублей и закрытие параллели]
  M -->|Нет| R5[Стабилизация, дополнительный цикл тестов]
  R5 --> L
  N --> O[Архив legacy и пост-аудит]
```
