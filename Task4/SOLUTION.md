...
**Текущее состояние.** Операции с данными преимущественно ручные; журналы, медкарты и платежи размазаны по Excel, сканам на **общем диске** и 1С; межсистемные потоки слабо формализованы; один физический контур без выделенной отказоустойчивости под прикладной уровень.
...
**Численные опоры для анализа узких мест (из целей кейса).** Рост объёма данных и клиентской базы **в пять раз**; появление **филиалов** и **мобильного** канала записи; переход с **одного** серверного контура и файлового режима 1С на **распределённую** архитектуру с API и очередями. Эти величины используются ниже как допущения для ёмкостного планирования и нагрузочных сценариев без выдумывания абсолютных SLA — конкретные p95 и доступность фиксируются после замеров на пилоте.
